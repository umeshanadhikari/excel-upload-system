...
//     return (
//       <div className="css-thwfws3" style={{ paddingLeft: '8px' }}>
...
//       // Prepare payload with proper string splitting
//       const payload = {
...
const Reports = () => {
  ...
  const CountValueContainer = ({ children, ...props }) => {
    const selectedCount = props.getValue().length; // Get the count of selected items
    return (
      <div style={{ paddingLeft: '8px', paddingRight: '8px' }}>
        {selectedCount > 0
          ? `${selectedCount} selected` // Display the count of selected items
  ...
  const MenuWithSelectAll = ({ children, ...props }) => {
    const options = props.options;
    const allSelected = props.getValue().length === options.length ; // -1 to exclude "Select All"
  ...
  const customStyles = {
    ...
    multiValue: () => ({
      display: 'none' // This will hide the selected items chips
    }),
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      const payload = {
        distributor: selectedDistributor.map(d => d.name).join(";"),
        salesRep: selectedSalesRep.map(s => s.name).join(";"),
