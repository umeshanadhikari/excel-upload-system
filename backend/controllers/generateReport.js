...
const generateReport = (data, filters, outputPath, months) => {
  return new Promise((resolve, reject) => {
    ...
    const baseFontSize = Math.max(8, 10 - Math.floor(monthCount / 6)); // Reduce font size as months increase
    ...
    Object.keys(distributorsData).forEach(distributor => {
      ...
      // Calculate column widths dynamically based on content (including first column)
      const summaryColumnWidths = summaryHeaders.map((header, i) => {
        ...
        // Calculate width needed for data
        let maxDataWidth = 0;
        distributorData.forEach(row => {
          let value = '';
          if (i === 0) {
            // First column - distributor/salesrep/agency
        ...
        // Use the wider of header or data, plus padding
      ...
      if (totalWidth > PAGE_WIDTH) {
        // Reduce all columns proportionally (including first column)
      ...
      const filteredDistributorData = distributorData.filter(row => {
        let agencyMatch = true;
        let productMatch = true;
        if (Array.isArray(filters.agency) && filters.agency.length > 0) {
          agencyMatch = filters.agency.includes(row.agency);
        }
        if (Array.isArray(filters.product) && filters.product.length > 0) {
          productMatch = filters.product.includes(row.product);
        }
      ...
      // Draw headers with dynamic height
      ...
      // Second Table - Product Breakdown (only for selected products/agencies)
      if ((filters.agency && filters.agency.length > 0) || (filters.product && filters.product.length > 0)) {
        const productHeaders = ["Product Name", ...monthNames, "Total Amount"];
        const productColumnWidths = summaryColumnWidths; // Use same widths as summary table

        // Group data by agency and product for this distributor
        const agencyProductData = {};
        filteredDistributorData.forEach(row => { 
          // Skip if this row doesn't match selected agencies/products
          if ((filters.agency && filters.agency.length > 0 && !filters.agency.includes(row.agency)) ||
              (filters.product && filters.product.length > 0 && !filters.product.includes(row.product))) {
            return;
          }
